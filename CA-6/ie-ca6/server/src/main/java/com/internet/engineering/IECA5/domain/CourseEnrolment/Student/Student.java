...
import com.internet.engineering.IECA5.repository.Course.CourseMapper;
import com.internet.engineering.IECA5.repository.Grade.GradeMapper;
    ...
    public void setSelectedCourses(List<WeeklyScheduleItem> selectedCourses) throws Exception {
        ...
        forEach(weeklyScheduleItem -> {
            try {
                ...
                MzRepository.getInstance().updateWeeklyScheduleItemVisibility(weeklyScheduleItem);
...
//                );
//
        for (WeeklyScheduleItem weeklyScheduleItem: MzRepository.getInstance().getNonFinalized(id))
    ...
    public List<WeeklyScheduleItem> getLastSubmit() {
        try {
            return MzRepository.getInstance().getLastSubmit(id);
        } catch (SQLException sqlException) {
            return null;
        }
    }
...
//        this.lastSubmit.add(weeklyScheduleItem);
    ...
    public double getGpa() throws Exception{
        ...
        for (Grade grade : MzRepository.getInstance().getAllGradesByStudentId(id)){
...
//    public void addGrade(Grade grade){this.grades.add(grade);}
    ...
    public double getTotalPassedUnits() throws Exception {
        ...
        for (Grade grade : MzRepository.getInstance().getAllGradesByStudentId(this.getStudentId()))
    ...
    public List<Grade> getGrades() {
        return MzRepository.getInstance().getAllGradesByStudentId(this.getStudentId());
    }

    public List<Grade> getGradesByTerm(int term) {
        return MzRepository.getInstance().getAllGradesByStudentId(this.getStudentId()).stream()
    ...
    public List<Term> getTerms() {
        ...
        Set<Integer> values = MzRepository.getInstance().getAllGradesByStudentId(this.getStudentId()).stream()
                .map(Grade::getTerm)
        ...
        for (Iterator<Integer> i = values.iterator(); i.hasNext();) {
            ...
            term.setGrades(MzRepository.getInstance().getAllGradesByStudentId(this.getStudentId()).stream()
